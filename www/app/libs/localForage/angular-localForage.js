    ...
    this.$get = ['$rootScope', '$q', '$parse', function($rootScope, $q, $parse) {
      ...
      LocalForageInstance.prototype.createInstance = function createInstance(config) {
        if(angular.isObject(config)) { // create new instance
		  var storeName = angular.isDefined(config.storeName) ? config.storeName : "keyvaluepairs";
          config = angular.extend({}, defaultConfig, config);          
          if(!angular.isDefined(lfInstances[config.name])) {
		    lfInstances[config.name] = {};
		  }
		  if(angular.isDefined(lfInstances[config.name][storeName])) {
            throw new Error('A localForage instance with the name ' + config.name + ' and store name ' + storeName + ' is already defined.');
          }
          lfInstances[config.name][storeName] = new LocalForageInstance(config);
          return lfInstances[config.name][storeName];
        } else {
